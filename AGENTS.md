...
- Use coarse-to-fine semantic discovery when the universe is too large for deep review. Cheap deterministic rules and cached source extracts should build issuer packets; low-reasoning or lightweight subagents may batch-classify many issuers for plain-English business description, bottleneck exposure, directness, small/early/awkward status, and need for deeper review; medium-reasoning subagents may compare candidates within a lane; xhigh reviewers remain reserved for material discovery conclusions, promotion, valuation, allocation, and unresolved conflicts.
...
## Discovery Cache And Compute Efficiency

The constitution controls the tradeoff. Discovery sensitivity, source freshness, auditability, and avoidable-ruin controls come before compute savings. Within that boundary, reduce repeated work aggressively because wasted retrieval, repeated filing extraction, duplicated subagent reading, and unnecessary xhigh review reduce the system's ability to search broadly for rare asymmetric outcomes.

Cache stable or expensive intermediate work when it is source-backed and auditable: SEC submissions metadata, filing manifests, filing text extraction, issuer profile packets, semantic batch classifications, historical prices, prior lane mappings, and prior rejection/incubation reasons. A cache entry is usable only when it records the source URL or durable identifier, source publication or data timestamp when available, retrieval or observation timestamp, input hash or source hash, selected scope, generation command or method, and freshness or invalidation rule.

Do not let cache become stale truth. Every material decision or full-cycle run must refresh or revalidate volatile surfaces: current prices, market cap, new SEC filings, company IR, financing and dilution, contracts, regulatory events, management changes, newly listed securities, spinoffs, direct listings, IPO and transaction filings, and material price dislocations. Cached issuer descriptions and old filing extracts may be reused to avoid rereading unchanged sources, but the cycle must still check whether newer sources exist.

Use change detection before recomputing. If a source hash, accession list, issuer identity, lane map, or market-data timestamp is unchanged and the cache is within its documented freshness window, reuse the cached extract and spend compute on uncovered issuers, changed sources, semantic gaps, same-lane comparisons, or higher-risk candidates. If the source changed, the cache is missing provenance, the cache scope is partial but being used as broad evidence, or the candidate could affect allocation, recompute or escalate rather than relying on the cache.

Token and subagent budgets should follow the funnel. Use deterministic code and low-reasoning batched subagents for broad semantic coverage; use medium reasoning for lane-level comparison and false-positive rejection; use xhigh only where judgment quality can materially change research readiness, watchlist priority, valuation, promotion, allocation, or risk. Record when a cheaper layer was sufficient and when escalation was required, so future runs can avoid both blind repetition and underpowered review.
