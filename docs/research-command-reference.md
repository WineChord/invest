...
Manual refreshes run during the U.S. session must not treat Yahoo's in-progress current-day candle as an official close. The refresh keeps the previous completed close until the provider's regular-session end timestamp has passed; missing session metadata also keeps the current market-date candle out of durable price, metric, and equity-snapshot files.

For SEC-derived fundamentals, the refresh rejects a materially stale instant share-count tag when a basic weighted-average share count from a filing period more than 180 days newer is available. The generated row labels this fallback. If a filing cover provides a newer exact basic share count that SEC companyfacts omits, add a dated and source-linked row to `data/market/share-count-overrides.csv`; the refresh uses it only while it is at least as current as the selected companyfacts value. Research notes should show diluted-share sensitivity separately when it is material instead of hiding it inside basic market capitalization.

A repeated refresh for the same completed close preserves an existing manually reconciled company-metric row. A later completed close can replace it with generated output, which is the signal to repeat the filing-based reconciliation for issuers with non-standard capital structures.
...
For every full operating cycle or material decision, run the event index and the default `foundational-first` filing index over the same current listed-security scope, then reconcile their selected accessions before concluding filing coverage is current. The event index answers what changed; the foundational index anchors the latest available 10-K, 20-F, S-1, F-1, or business prospectus. A newer foundational filing must not remain hidden behind an older event filing selected for event review. Targeted cycles may narrow the paired scope only to the affected targets, holdings, and material opportunity-cost comparators, and must label that scope as partial.

The event index is not a complete issuer-change or foundational-business scan. Every material decision and full cycle must also compare current SEC submissions against the prior decision for ownership, compensation, financing, and private-offering forms that the selector intentionally omits, including Form 3, Form 4, Form 144, Schedule 13D/13G amendments, Form D, DEFA14A, and S-8. Record material results in `research/freshness/events.csv`; explicitly classify immaterial results rather than assuming an empty event index means no change.
...
The SEC submissions builders support `--submissions-cache-dir`, `--cache-only` or `--require-cached-submissions`, `--request-delay-ms`, `--max-submissions-cache-age-days`, bounded retry controls, and optional `--submissions-ledger-output`; live complete-universe runs require at least a 100 ms request delay unless they are cache-only. Cached submissions are validated against the SEC company list CIK, ticker, exchange, cache freshness, and non-future cache observation time before use. A stale but otherwise valid cache must be refetched during an online run and must fail during cache-only operation; staleness must not abort a live refresh before the network fallback is attempted. Complete artifacts record a per-CIK submissions ledger with cache status, source URL, payload SHA-256, cache observed timestamp, cache age, validation status, request attempts, retrieval timestamp, fetch timestamp when applicable, and SEC user-agent metadata. When `--submissions-ledger-output` is provided, the ledger is written incrementally so failed cache validation or failed fetches still leave a resumable audit artifact.

Remote filing HTML extraction supports `--filing-cache-dir`, `--cache-only` or `--require-cached-filings`, `--max-filing-cache-age-days`, bounded retry controls, and optional `--filing-ledger-output`; cache hits require sidecar metadata with source URL, fetch timestamp, retrieval date, payload SHA-256, and SEC user-agent, so copied cache files cannot become fresh by filesystem mtime alone. A stale filing cache follows the same rule as submissions: refetch online, fail cache-only. Profile artifacts record per-filing cache status, request attempts, request statuses, payload hashes, retrieval dates, and fetch timestamps so filing-content provenance remains auditable.
...
Large semantic-discovery work orders are different: write issuer packets, semantic batch files, smoke artifacts, validation artifacts, dated cache snapshots, and full complete-universe SEC issuer-profile dumps under ignored `research/cache/discovery/YYYY-MM-DD/`, then commit only durable summaries, hashes, source metadata, and classification cache records that are small enough to remain reviewable. Semantic cache freshness is tied to packet hash, lane-map hash, schema version, classifier version, and explicit cache validity; changing classifier logic should invalidate old rows instead of silently reusing them. A durable semantic run must bind to an immutable dated snapshot of the cache used for that run, never directly to the mutable shared cache. The snapshot stays ignored; the run artifact preserves its path and SHA-256. `check:data` rejects cache-only intermediates if they are left under `research/discovery/runs/`, validates semantic review packets against the semantic run they summarize, and fails if ignored `research/cache/` or `research/downloads/` files become tracked.
...
cp research/discovery/semantic-cache.jsonl research/cache/discovery/YYYY-MM-DD/semantic-cache-snapshot.jsonl
npm run build:semantic-discovery-run -- --as-of YYYY-MM-DD --packets research/cache/discovery/YYYY-MM-DD/semantic-packets.json --cache research/cache/discovery/YYYY-MM-DD/semantic-cache-snapshot.jsonl --output research/discovery/runs/YYYY-MM-DD-semantic-discovery-run.json
...
npm run test:standing-contribution
npm run verify
```
