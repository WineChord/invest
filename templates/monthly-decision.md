...
- Build a dated evidence packet for advisory reviewers, using `npm run build:evidence-packet -- --as-of YYYY-MM-DD --output research/discovery/runs/YYYY-MM-DD-subagent-evidence-packet.yml` when practical. The packet should include policy version, confirmed account facts, freshness window, candidate set, source publication dates, retrieval dates, deterministic outputs, open freshness events, valuation states, quality metrics, allowed assets, and specific questions.
