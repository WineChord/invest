...
- Use `npm run scan:community` as a public no-token community lead scan across configured Reddit RSS/Atom feeds, Stocktwits public endpoints, Hacker News Algolia search, and generic RSS feeds. The scan can surface attention shifts, cash-tag mentions, and bottleneck-lane keyword mentions, but it is sentiment and lead generation only. It must not create buy eligibility, promotion eligibility, security metadata, freshness evidence, or allocation evidence by itself; any material symbol from the scan must still pass security-type confirmation, primary-source skim, discovery readiness, and promotion gates as applicable.
