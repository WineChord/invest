...
Release preflight must inspect the exact staged payload, including newly added files, and pass `git diff --cached --check` alongside the applicable validation and sensitive-field review. An unstaged diff does not cover new files. Deferred delivery must stop unattended retries on deterministic validation failures or a changed reviewed payload or remote base; preserve the failure, reconcile and review the changes, then resume explicitly. Retry transient delivery failures only while the release gates still pass.
...
- When a non-held symbol cannot be refreshed because a provider request fails, the refresh must preserve that symbol's existing committed price history, technical snapshot, and company-metric row rather than erasing coverage. The run must report the failed symbol, and a later successful refresh may replace the preserved rows normally.
- Yahoo may expose an in-progress daily candle during the current New York trading session. The refresh must exclude the current market-date candle until Yahoo's regular-session end timestamp has passed; if the session metadata needed to prove completion is absent, the current market-date candle must be skipped. Only completed daily bars may update price snapshots, technical metrics, company valuation metrics, or the equity curve.
- SEC-derived market capitalization must not let a stale instant share-count tag override a materially newer filing-period share count. When the instant fact is more than 180 days older than the latest basic weighted-average fact, the refresh must use the newer basic period average and label the fallback. When an issuer filing cover supplies a newer exact basic share count that SEC companyfacts omits, `data/market/share-count-overrides.csv` may supply that dated, source-linked value; the refresh must prefer it only while it is at least as current as the selected companyfacts value. Diluted-share sensitivity belongs in explicit research rather than being silently substituted for basic market capitalization.
- When a company-metric row has been manually reconciled from primary filings, a repeated refresh for the same completed close must preserve that curated row instead of replacing it with the generic SEC/FMP output. A later completed close may generate a new row and therefore requires a fresh reconciliation when the capital structure remains non-standard; this safeguard prevents no-new-data weekend or rerun refreshes from silently erasing reviewed evidence without freezing old price-dependent metrics indefinitely.
...
Independent-review coverage is recorded with `required_xhigh_roles`, `completed_xhigh_roles`, `skipped_xhigh_roles`, and `skipped_xhigh_role_reasons`. Every required role must be either completed or explicitly skipped for one of the allowed reasons in `AGENTS.md`; completed and skipped sets must not overlap. Use `partial_tool_unavailable` when one or more required roles could not run because the tool was unavailable, `partial_allowed_skips` when all skips have another permitted reason, and `complete` only when no required role was skipped. An allowed, disclosed skip does not by itself make a primary-evidence-supported decision incapable of recommending a buy, and it must not be represented as independent consensus.

For a promotion transition, `xhigh_roles_completed` records only actual independent xhigh completions. Missing required roles may be resolved by explicit permitted skips in `subagent_review_path` when that structured run has the same decision date and binds the same symbol and promotion-review path. The validator verifies claimed completions, rejects duplicate or contradictory roles, and accepts only the existing `AGENTS.md` skip reasons. It treats `evidence_freshness` as the promotion name for `freshness_filing_review`, and `opportunity_cost_allocation` as the promotion name for `allocation_risk`. Historical links without bound structured role records retain the strict completed-role requirement.
...
`research/community-sources.yml` records public no-token community sources that can be used for weak sentiment and lead-generation scans. It must not require cookies, API tokens, logged-in browser state, private communities, paid scraping services, or unstable web-scraping workflows. X is excluded from the no-token source list unless a later workflow uses an approved credentialed API boundary.
...
- `scripts/build-semantic-discovery-run.mjs` for summarizing the current semantic cache against the current packet artifact. Before building the durable run, copy the mutable shared cache to an ignored immutable dated snapshot and pass that snapshot to the builder; the run must preserve the snapshot path and SHA-256 so later shared-cache updates cannot invalidate historical evidence. It reports classifier version, classification coverage, stale cache count, unclassified symbols, lane counts, medium-review queue, xhigh-readiness queue, rejection/archive queue, a no-escalation sample, and required next steps. This summary is triage, not buy eligibility.
